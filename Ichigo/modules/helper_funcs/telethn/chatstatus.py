from Ichigo.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Ichigo import DRAGONS
...
async def ichigo_is_admin(chat_id: int):
    status = False
    ichigo = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if ichigo.id == user.id:
            status = True
            break
    return status
